...
# IMPORTANT NOTE: If texlive-full installation freezes, keep pressed enter key until unlock

echo 'Installing all the packages' && \
sudo apt install -y git tree vim maven gradle wireless-tools net-tools gnome-shell-extensions preload usb-creator-gtk \
	...
	font-manager python3-pip python3-dev gnome-tweaks build-essential libusb-1.0-0-dev \
	libudev-dev tcpdump traceroute gnome-disk-utility ipcalc rhythmbox synaptic pulseaudio \
	obs-studio mesa-vulkan-drivers nvidia-settings vulkan-tools speedtest-cli piper \
	ntpdate htop nvtop vlc mysql-server gparted libreoffice bleachbit zsh libssl-dev \
	lm-sensors psensor bat boot-repair qdirstat musl xclip ffmpeg pciutils unrar file-roller \
	whois docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin docker-compose brave-browser \
	linux-tools-common linux-tools-generic linux-tools-"$(uname -r)" valgrind cmake krita \
	texlive-full biber texlive-lang-spanish texlive-extra-utils libomp-dev libopenmpi-dev \
 	libasound2-dev mesa-common-dev libx11-dev libxrandr-dev libxi-dev xorg-dev libgl1-mesa-dev inkscape \
  	libglu1-mesa-dev libopenal-dev libdw-dev libevent-dev libhwloc-dev libpmix-dev cmatrix libncurses6 libfuse2t64 && \
...
echo "Comment lines in the '/etc/apt/sources.list.d/ookla_speedtest-cli.list' file"
